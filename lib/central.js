...
autostatic.express = express;

jade.doctypes.default = '<!DOCTYPE html>';
istatic.default({
  js: {
    filter: function(str) {
      return str.replace(/_log\(.*?\)/, '');
    }
  },
  css: {
    filter: function(str) {
      return str.replace(/url\([\'\"](.*?)\1\)/, function(p0, p1) {
...
if (conf.upyun) {
  ...
  function retry(args) {
    // retrid five times
    if (args[0].counter > 5) return;
    central.log('warn', '[upyun upload] retry.. ', path, hash);
    upyUpload.apply(central.autostatic.manager, args);
  }

  function upyUpload(path, contents, fn, hash) {
    var args = arguments;
    var this_fn = args.callee;

    upyun.setContentMD5(hash || null);
    upyun.writeFile(path, contents, true, function(err, data) {
      if (err) {
        central.log('error', '[upyun upload] -', path, err);
        central.log('error', '\n\n');
        central.log('error', data, contents);

        path.counter = (path.counter && path.counter++) || 1;
  ...
  var staticManager = central.autostatic.manager;
  central.autostatic.manager.upload = function(p, contents, fn, hash) {
    var m_p = staticManager.getMin(p);
    if (m_p == p) return fn();
    fs.writeFile(m_p, contents, fn);
  };
}
