...
//express.autostatic = autostatic;
autostatic.static = express.static;

autostatic.set({
  direct: '/assets',
  debug: conf.debug,
  uri_root: conf.assets_root
});
  ...
  autostatic.set({
    ...
    upload: function(path, contents) {
      upyun.writeFile(path, contents, true, function(err, data) {
        if (err) return central.log('error', '[upyun upload]', path, err, data, contents);
        central.log('info', '[upyun]', path, 'uploaded');
        // mark this file as ready, so we can serve it from cdn.
        autostatic.markReady(path, 10);
      });
    }
  });
}
