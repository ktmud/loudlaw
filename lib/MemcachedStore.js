...
MemcachedStore.prototype.get = function(sid, fn) {
  ...
  self.client.get(sid, function(err, data) {
    try {
      if (!data) {
        return self.db ? self.rawData(sid, fn) : fn();
      }
...
MemcachedStore.prototype.rawData = function(sid, fn) {
  var self = this;
...
MemcachedStore.prototype.set = function(sid, sess, fn) {
  ...
  try {
    ...
    if (self._changes && ttl > self.sync_time) {
  ...
  setTimeout(function() {
    ...
    db.query({
      ...
      for (var sid in changes) {
        ...
        // delete this session
        if (val === null) obj._deleted = true;
        if (revs[sid]) obj._rev = revs[sid];
        arr.push(obj);
      }
      arr.length && db.save(arr, function(err, res) {
